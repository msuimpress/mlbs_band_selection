...
DATASE_ADDRESST = ["hbs_mlbs_"+DATASET+"\\image_train_"+DATASET+".npy",
                   ...
                   "hbs_mlbs_"+DATASET+"\\mask_"+DATASET+".npy",
...
mask_train = np.load(DATASE_ADDRESST[2])[:image_train.shape[0]]
...
layer_mask = tf.keras.Input(shape=(num_band,1))
...
z = layer_sampled_mask([y, layer_mask])
t = layer_proxy_data([layer_input, z])
measurement_model = tf.keras.Model(inputs = [layer_input, layer_mask], outputs = t)

measurements_train = measurement_model.predict([image_train, mask_train])
...
image_test = np.load(DATASE_ADDRESST[3])
label_test = np.load(DATASE_ADDRESST[4])
label_test_int = np.argmax(label_test, axis=1)
mask_test = np.tile(mask_train[0][np.newaxis,:,:], (image_test.shape[0],1,1))

measurements_test = measurement_model.predict([image_test, mask_test])
